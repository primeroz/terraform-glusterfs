...
00 * * * * root cd /mnt/gluster/test1/sysbench && sysbench --test=fileio --file-total-size=8G prepare
15 * * * * root cd /mnt/gluster/test1/sysbench && sysbench --test=fileio --file-total-size=8G --file-test-mode=seqrd --max-time=300 run
20 * * * * root cd /mnt/gluster/test1/sysbench && sysbench --test=fileio --file-total-size=8G --file-test-mode=seqwr --max-time=300 run
25 * * * * root cd /mnt/gluster/test1/sysbench && sysbench --test=fileio --file-total-size=8G --file-test-mode=seqrewr --max-time=300 run
